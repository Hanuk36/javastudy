...
/*
   ...
   - 람다식 : 함수형 프로그램에서 사용하는 방식으로
     ...
     *** 추상 메서드가 하나 이상이면 어떤 메서드를 구현하는 것인지 모호해진다.
         이름 방지 위해서 @Functionalinterface 에노테이션 사용해서
         오류를 발생하게 한다.
         
     람다식 -> 매게변수를 가진 코드블록 -> 익명 구현 객체
     람다식 형식) : ([매게변수]) -> {실행문};
     
           일반메서드                     -->       람다식 : 1)반환형과 이름 삭제, 2) ([매게변수]) -> {}
	 ...
	 3. return 생략하기
	    구현부분의 return 문 하나라면 중괄호 와 return 문 모두 생략하고 식만 씁니다.
	    (x,y) -> x+y;
	    x -> x+10
