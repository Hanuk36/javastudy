...
/*
  ...
  1-2. 결합스트림: 바이트 스트림 => 문자 스트림 (둘 사이에 있는것)
           기계를 통헤 입/출력된 정보를 사람이 알아볼 수 있도록 입/출력 한다.
       최상위 클래스: InputStreamReader(입력), OutputStreamWriter(출력)
...
public class Ex08 {
	public static void main(String[] args) {
		...
		//                              **String 클래스에 getByte()를 이용하면 byte[]로 만들어진다.
		//             flush(): 출력 버퍼 용량이 다 차지 않아도 바로 출력하게 만드는 메서드
		//             close(): 출력 스트림 닫기
		...
		File file = new File(pathname);
		FileOutputStream fos = null; // fos.close();를 위하여 밖으로 뺀것임
		try {
			//파일이 존재하면 덮어쓰기가 된다. 파일이 존재하지 않으면 생성된다.
			fos = new FileOutputStream(file);
			//java 라는 글자가 test01 안에 만들어졌다
			fos.write(106); //j
			fos.write(97); //a
			fos.write(118); //v
			fos.write(97);  //a
