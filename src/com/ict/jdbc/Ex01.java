...
// 1. 자바와 오라클을 연결해주는 드라이버가 필요하다. (ojdbcXXX.jar)가 필요하다.
// 그런데 sqldeveloper 에 존재한다. 해당 드라이버를 찾아서 util 폴더에 넣자.
// 2. 이클립스에서 프로젝트 마우스 오른쪽 Build Path -- Configure Build Path 
// 여기까지가 사용하기 위한 준비단계

public class Ex01 {
	public static void main(String[] args) {
		// 1.java 를 오라클에 접속할수 있도록 도와 주는 클래스
		Connection conn = null;
		// 2.SQL 구문 작성
		Statement stmt = null;
		// 3-1. 결과를 받는 클래스(select 일때)
		ResultSet rs = null;
		// 3-2. 결과를 받은 변수(select 가 아닐때)
		int result = 0;
		try {
